...
class _OpnameSessionFormPageState extends State<OpnameSessionFormPage>
  ...
  void _shareFile() {
    shareFile(opnameSession).then((result) {
      if (result == null) {
        toastification.show(
  ...
  final _opnameSessionFormState = GlobalKey<FormState>();
  static const labelStyle = TextStyle(fontWeight: .bold, fontSize: 16);

  Future<bool> hasOpenSession(String location) async {
    final orm = Orm(
      tableName: OpnameSession.tableName,
      pkField: OpnameSession.pkField,
      db: db,
    );
    final sameOpnameSession = await orm.findBy({
      'location': location,
      'status': OpnameStatus.open.toString(),
    }, OpnameSession.convert);
    return sameOpnameSession != null &&
        opnameSession.id != sameOpnameSession.id;
  }
        ...
        child: Padding(
          ...
          child: Center(
            child: Column(
              ...
              children: [
                const Text('Sesi Opname', style: labelStyle),
                const SizedBox(height: 10),
                Form(
                  key: _opnameSessionFormState,
                    ...
                    children: [
                      ...
                      FormField<String>(
                        ...
                        onSaved: (newValue) {
                          opnameSession.location =
                              newValue ?? opnameSession.location;
                        },
                        initialValue: opnameSession.location,
                        builder: (state) => DropdownMenu<String?>(
                          ...
                          errorText: state.errorText ?? locationError,
                          onSelected: (value) {
                            setState(() {
                              locationError = null;
                            });
                      ...
                      DropdownMenu<OpnameStatus>(
                        ...
                        dropdownMenuEntries: OpnameStatus.values
                            .map<DropdownMenuEntry<OpnameStatus>>(
                              (status) => DropdownMenuEntry<OpnameStatus>(
                                value: status,
                ...
                ElevatedButton(
                  onPressed: () async {
                    if (_opnameSessionFormState.currentState?.validate() !=
                        true) {
                      return;
                    }
                    _opnameSessionFormState.currentState?.save();
                    if (await hasOpenSession(opnameSession.location)) {
                      setState(() {
                        locationError = 'sudah ada sesi yang aktif';
                      });

                      return;
                    }
                    await saveOpnameSession();
                  },
                ...
                Visibility(
                  visible: !opnameSession.isNewRecord,
                  child: Form(
                    key: _formState,
                    autovalidateMode: .always,
                    child: Wrap(
                ...
                Visibility(
                  visible: !_isFetchingItem && !opnameSession.isNewRecord,
                  child: SizedBox(
  ...
  void _checkCode(String? value) async {
    ...
    _openInputQuantityModal(item, focusNode: focusNode2)
        .then((int? quantity) {
          if (quantity != null) {
  ...
  Future<int?> _openInputQuantityModal(
    Item item, {
    FocusNode? focusNode,
    int? quantity,
    OpnameItem? opnameItem,
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const Divider(),
                Visibility(
                  visible: opnameItem != null,
                  child: Padding(
                    padding: const EdgeInsets.only(top: 10),
                    child: SizedBox(
                      width: 300,
                      child: TextFormField(
                        initialValue: opnameItem?.rack.join(', '),
                        validator: (value) {
                          if (value == null || value.isEmpty) {
                            return 'harus diisi';
                          }
                ...
                Row(
                  children: [
                    ElevatedButton(
                      onPressed: () => onSubmitted(focusNode, item.code),
  ...
  Future<OpnameSession> saveOpnameSession() async {
    final orm = Orm(
      tableName: OpnameSession.tableName,
      pkField: OpnameSession.pkField,
      db: db,
    );
    await orm.save(opnameSession);
    setState(() {
      opnameSession;
    });
  ...
  void _updateOpnameItem(
    OpnameItem opnameItem, {
    required int quantity,
    bool addRackFromInput = true,
  }) {
    final orm = Orm(
      tableName: OpnameItem.tableName,
      pkField: OpnameItem.pkField,
    ...
    if (rack.isNotEmpty && addRackFromInput) {
